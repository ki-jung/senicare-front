...
export const patchNurseRequest = async (reqeustBody: PatchNurseRequestDto, accessToken: string) => {
    const responseBody = await axios.patch(PATCH_NURSE_API_URL, reqeustBody, bearerAuthorization(accessToken))
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
}
